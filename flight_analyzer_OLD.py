...
class FlightAnalyzer:
                    ...
                    if not phase_data.empty:
                        ...
                        for idx in anomaly_indices:
                            anomalies_detected.append({
                                'flight_id': flight_df.loc[idx, 'flight_id'],
                                'parameter': param,
                                'phase': phase,
    ...
    def analyze_flight(self, excel_path, sheet_name='Clean Data', add_to_training=False):
        ...
        try:
            ...
            # Convert anomalies to native Python types
            anomalies = self._convert_to_native_types(anomalies)
            ...
            for phase in phases:
                ...
                if not phase_data.empty and param in phase_data.columns:
                    ...
                    viz_data[param][phase] = {
                        'normal': {
                            'time': normal_data['_time'].tolist(),
                            'values': normal_data[param].tolist()
                        },
                        'anomalies': {
                            'time': anomaly_data['_time'].tolist(),
                            'values': anomaly_data[param].tolist()
                        },
                        'has_data': len(phase_data) > 0
                    }
                else:
    ...
    def _get_phases_summary(self, flight_df, anomalies):
        ...
        for phase in phases:
            ...
            summary[phase] = {
                ...
                'anomaly_count': len(phase_anomalies),
                'anomaly_percentage': round((len(phase_anomalies) / len(phase_data)) * 100, 2) if len(phase_data) > 0 else 0
            }
